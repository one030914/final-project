document.addEventListener("DOMContentLoaded", function () {
    ...
    me.fromTo(".me",// 1. `.me` 整体渐显动画
        { opacity: 0 },
        { opacity: 1 }
    ).fromTo(".me img",// 2. `.me img` 出现和消失动画
        { y: 100, opacity: 0 }, // 出现前：向下偏移 100px，透明
        { y: 0, opacity: 1 }, // 出现后：恢复原位，完全显示
        "<",
    ).fromTo(".gradient-overlay2",// 4. `.gradient-overlay2` 出现和消失动画
        { y: 100, opacity: 0 }, // 出现前：向下偏移 100px，透明
        { y: 0, opacity: 1 }, // 出现后：恢复原位，完全显示
        "<",
    ).fromTo(".info",// 3. `.info` 出现和消失动画
        { y: 100, opacity: 0 }, // 出现前：向下偏移 100px，透明
        { y: 0, opacity: 1 }, // 出现后：恢复原位，完全显示
        "<",
    );
